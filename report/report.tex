...
Минимальные условия для обоих методов:
\begin{itemize}
    \item $\exists x_0: f(x_0) = 0$
    \item $f(x)$ непрерывно дифференцируема на отрезке и в его граничных точках слева и справа
...
Площадь заданной фигуры должна быть вычислена с точность $\varepsilon$. Пусть производная функции $f_i$ ограничена на отрезке поиска корня некоторым значением $M_i$: $-M_i \leq f_i'(x) \leq M_i$ (следует из непрерывности производной). Тогда в некоторой окрестности точки пересечения, которую мы ищем с точностью до $\varepsilon_1$, функции эквивалентны своим касательным в этой точке.~\cite{math} Значит, при смещении границы интегрирования, изменение значения интеграла будет пропорционально квадрату смещения:\\\\
$\Delta S = \frac{1}{2} \cdot \varepsilon_1 \cdot (|f_i'(x_0)| + |f_j'(x_0)|) \cdot \varepsilon_1 \leq \frac{M_i + M_j}{2} \cdot \varepsilon_1^2$.\\
...
\begin{axis}[% grid=both,                % рисуем координатную сетку (если нужно)
             ...
             restrict y to domain=0:3,  % задаем диапазон значений функции y(x)
...
\addplot[green,samples=256,thick,name path=A] {x^2};
...
\addplot[green,samples=256,thick,name path=C] {2*x - 1};

\addplot[blue,samples=256,thick,name path=D] {2 - x};
...
\addplot[color=black] coordinates {(1, 0)} node [label={-90:{\small $x_0$}}]{};

\addplot[dashed] coordinates { (0.6, 1.67) (0.6, 0) };
\addplot[color=black] coordinates {(0.6, 0)} node [label={-90:{\small $x_0 - \varepsilon_1$}}]{};

\addplot[dashed] coordinates { (0.6, 1) (1, 1) };

% закрашиваем фигуру
\addplot[blue!20,samples=256] fill between[of=C and D,soft clip={domain=0.6:1}];
    \addlegendentry{$\Delta S=\frac{1}{2} \cdot \varepsilon_1 \cdot (f_1'(x_0) + f_2'(x_0)) \cdot \varepsilon_1$}
...
Общая площадь фигуры равна комбинации трех интегралов с некоторыми знаками; общая погрешность может достигать $3\varepsilon_2 + \varepsilon_1^2 \cdot \frac{2M_1 + 2M_2 + 2M_3}{2} \leq 0.001$ по условию.\\
Возьмем $\varepsilon_2 = 0.0001$, тогда $\varepsilon_1^2 \cdot (M_1 + M_2 + M_3) \leq 0.0007$, $\varepsilon_1 \leq \frac{\sqrt{0.0007}}{\sqrt{M_1 + M_2 + M_3}} = \frac{0.026457513}{\sqrt{M_1 + M_2 + M_3}}$.\\

Для вычисления интеграла с определенной точностью было использовано правило Рунге. Метод Симпсона имеет порядок точности 4, метод трапеций имеет порядок точности 15. Оценка погрешности равна разнице между двумя площадями, посчитанными с разной точностью, деленной на порядок точности. Поэтому количество разбиений в обоих методах экспоненциально повышается, пока оценка не станет меньше $\varepsilon_2$. ~\cite{math}\\

После нахождения точек, сначала определяется верхняя/нижняя ограничивающая кривая. Это кривая, которая не пересекает другие в средней из трех (по оси $x$) точке пересечения. Определив, лежит ли она ниже или выше этой точки, мы либо вычитаем ее интеграл из интегралов двух других функций, либо наоборот:

\[
S = sgn(f_1(x_1) - f_2(x_1)) \cdot (\int_a^b f_1(x)\,dx - \int_a^{x_1} f_2(x)\,dx - \int_{x_1}^b f_3(x)\,dx).\\\\
\]
...
\subsection{Запуск}
В директории bin лежат два файла -- main и gen. Файл main -- основная программа, gen -- компилятор функций в ассемблер. Обе программы выводят поддерживаемые флаги комадной строки через --help. В main через флаг -m можно задать верхние границы производных на отрезке (10000 по умолчанию), которые, как предполагается, предварительно просчитаны аналитически. В gen по флагу --test можно запустить тестовый режим: на stdin подается одна функция в RPN, программа выводит ее компиляцию в NASM. Файл test.sh собирает и запускает программу для нескольких примеров из директории examples.\\\

Формат ввода функций поддерживает константы e и pi. Также поддерживается унарный минус, но только для констант (<<-1 x *>> -- работает, <<-x>> -- не работает, <<x ->> -- не работает).\\
