...
\subsection{Запуск}
В директории bin лежат два файла -- main и gen. Файл main -- основная программа, gen -- компилятор функций в ассемблер. Обе программы выводят поддерживаемые флаги комадной строки через --help. В main через флаг -m можно задать верхние границы производных на отрезке (10000 по умолчанию), которые, как предполагается, предварительно просчитаны аналитически. В gen по флагу --test можно запустить тестовый режим: на stdin подается одна функция в RPN, программа выводит ее компиляцию в NASM.\\\\
Файл test.sh собирает и запускает программу для нескольких примеров из директории examples.
