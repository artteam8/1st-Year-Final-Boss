...
Площадь заданной фигуры должна быть вычислена с точность $\varepsilon$. Пусть производная функции ограничена на отрезке поиска корня некоторым значением $M$: $-M \leq f'(x) \leq M$. Тогда в некоторой окрестности точки пересечения, которую мы ищем с точностью до $\varepsilon_1$, функции эквивалентны своим касательным в этой точке.~\cite{math} Значит, при смещении границы интегрирования, изменение значения интеграла будет пропорционально квадрату смещения:\\\\
$\Delta S = \frac{|f'(x_0)|}{2} \cdot \varepsilon_1^2 \leq \frac{M}{2} \cdot \varepsilon_1^2$.\\

Общая площадь фигуры равна комбинации трех интегралов с некоторыми знаками; общая погрешность может достигать $3\varepsilon_2 + 2\varepsilon_1^2 \cdot \sum_{i=1}^3 \frac{M_i}{2} \leq 0.001$ по условию.\\
Возьмем $\varepsilon_2 = 0.0001$, тогда $\varepsilon_1^2 \cdot \sum_{i=1}^3 M_i \leq 0.0007$, $\varepsilon_1 \leq \frac{\sqrt{0.0007}}{\sqrt{\sum_{i=1}^3 M_i}} = \frac{0.026457513}{\sqrt{\sum_{i=1}^3 M_i}}$.\\
...
\subsection{Запуск}
В директории bin лежат два файла -- main и gen. Файл main -- основная программа, gen -- компилятор функций в ассемблер. Обе программы выводят поддерживаемые флаги комадной строки через --help. В main через флаг -m можно задать максимумы производных на отрезке (10000 по умолчанию). В gen по флагу --test можно запустить тестовый режим: на stdin подается одна функция в RPN, программа выводит ее компиляцию в NASM.\\\\
